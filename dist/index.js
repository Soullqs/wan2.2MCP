...
import { SUPPORTED_SIZES, SUPPORTED_STYLES, SUPPORTED_QUALITIES } from './types/index.js';
...
class Wan2MCPServer {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            try {
                ...
                switch (name) {
                    ...
                    case 'list-history':
                        return await this.handleListHistory(args);
                    case 'test-model':
                        return await this.handleTestModel(args);
                    case 'diagnose-api':
                        return await this.handleDiagnoseApi(args);
                    default:
    ...
    async handleGenerateImage(params) {
        ...
        if (!this.configService.isConfigValid()) {
            throw new McpError(ErrorCode.InvalidRequest, 'API key not configured. Please use set-config tool first.');
        }
        // 初始化DashScope客户端
        ...
        try {
            ...
            // 创建历史记录
            historyRecord = await this.historyService.createRecord(fullParams.prompt, fullParams.size, fullParams.style, fullParams.quality, fullParams.n, response.output.task_id);
            // 等待任务完成
            const completedResponse = await this.dashScopeClient.waitForCompletion(response.output.task_id);
            // 提取图像URL
    ...
    async handleListHistory(params) {
        ...
        const recordsText = history.records.map((record, index) => `${index + 1}. [${record.status.toUpperCase()}] ${record.prompt.substring(0, 50)}${record.prompt.length > 50 ? '...' : ''}\n` +
     ...
     * 处理API诊断请求
     */
    async handleDiagnoseApi(params) {
        const config = this.configService.getConfig();
