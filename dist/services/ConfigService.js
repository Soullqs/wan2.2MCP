...
export class ConfigService {
    ...
    getConfig() {
        if (!this.config) {
            throw new MCPError('Config not loaded. Call initialize() first.', 'CONFIG_NOT_LOADED');
        }
        return { ...this.config };
    }
    /**
    ...
    async updateConfig(params) {
        try {
            ...
            if (!params.api_key || params.api_key.trim() === '') {
                throw new MCPError('API key is required', 'INVALID_API_KEY');
            }
            // 更新配置
            const updatedConfig = {
                ...this.config,
    ...
    validateConfig(config) {
        ...
        if (!validRegions.includes(config.region)) {
            throw new MCPError(`Invalid region: ${config.region}. Supported regions: ${validRegions.join(', ')}`, 'INVALID_REGION');
        }
        // 验证尺寸
        ...
        if (!validSizes.includes(config.default_size)) {
            throw new MCPError(`Invalid size: ${config.default_size}. Supported sizes: ${validSizes.join(', ')}`, 'INVALID_SIZE');
        }
        // 验证风格
        ...
        if (!validStyles.includes(config.default_style)) {
            throw new MCPError(`Invalid style: ${config.default_style}. Supported styles: ${validStyles.join(', ')}`, 'INVALID_STYLE');
        }
        // 验证质量
        const validQualities = ['standard', 'hd'];
        if (!validQualities.includes(config.default_quality)) {
            throw new MCPError(`Invalid quality: ${config.default_quality}. Supported qualities: ${validQualities.join(', ')}`, 'INVALID_QUALITY');
        }
    }
    /**
     * 检查配置是否有效
    ...
    getApiKey() {
        const config = this.getConfig();
        if (!config.api_key) {
            throw new MCPError('API key not configured. Please set API key first.', 'API_KEY_NOT_CONFIGURED');
        }
