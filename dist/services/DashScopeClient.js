...
export class DashScopeClient {
    ...
    apiKey;
    baseURL;
    constructor(apiKey, region = 'cn-beijing') {
        this.apiKey = apiKey;
    ...
    handleApiError(error) {
        if (error.response) {
            ...
            switch (status) {
                case 400:
                    return new MCPError(`Bad request: ${message}`, 'DASHSCOPE_BAD_REQUEST', data);
                case 401:
                    return new MCPError('Invalid API key or authentication failed', 'DASHSCOPE_AUTH_ERROR', data);
                case 403:
                    return new MCPError('Access forbidden. Check your API key permissions.', 'DASHSCOPE_FORBIDDEN', data);
                case 429:
                    return new MCPError('Rate limit exceeded. Please try again later.', 'DASHSCOPE_RATE_LIMIT', data);
                case 500:
                    return new MCPError('DashScope server error. Please try again later.', 'DASHSCOPE_SERVER_ERROR', data);
                default:
                    return new MCPError(`DashScope API error: ${message}`, 'DASHSCOPE_API_ERROR', data);
            }
        }
        else if (error.code === 'ECONNABORTED') {
            return new MCPError('Request timeout. The image generation is taking too long.', 'DASHSCOPE_TIMEOUT');
        }
        else {
            return new MCPError(`Network error: ${error.message}`, 'DASHSCOPE_NETWORK_ERROR', error);
        }
    }
    /**
     * 生成图像
    ...
    async generateImage(params) {
        ...
        try {
            // 发送请求
            const response = await this.client.post('/api/v1/services/aigc/text2image/image-synthesis', params);
            const result = response.data;
            // 验证响应
            if (!result.output) {
                logger.error('Invalid response from DashScope API', 'DashScopeClient', { result });
                throw new MCPError('Invalid response from DashScope API: missing output', 'DASHSCOPE_INVALID_RESPONSE', result);
            }
            ...
            throw new MCPError(`Failed to generate image: ${error instanceof Error ? error.message : 'Unknown error'}`, 'DASHSCOPE_GENERATION_ERROR', error);
        }
    }
    /**
     * 查询任务状态
    ...
    async getTaskStatus(taskId) {
        ...
        try {
            ...
            throw new MCPError(`Failed to get task status: ${error instanceof Error ? error.message : 'Unknown error'}`, 'DASHSCOPE_STATUS_ERROR', error);
        }
    }
    /**
     * 等待任务完成
     */
    async waitForCompletion(taskId, maxWaitTime = 300000, // 5分钟
    pollInterval = 3000 // 3秒
            ...
            try {
                ...
                switch (result.output.task_status) {
                    ...
                    case 'FAILED':
                        ...
                        throw new MCPError(`Image generation failed for task ${taskId}`, 'DASHSCOPE_TASK_FAILED', result);
                    case 'PENDING':
                    case 'RUNNING':
                    ...
                    default:
                        ...
                        throw new MCPError(`Unknown task status: ${result.output.task_status}`, 'DASHSCOPE_UNKNOWN_STATUS', result);
                }
            }
            catch (error) {
                if (error instanceof MCPError) {
        ...
        throw new MCPError(`Task ${taskId} did not complete within ${maxWaitTime / 1000} seconds`, 'DASHSCOPE_TIMEOUT');
    }
    /**
     * 睡眠函数
     */
